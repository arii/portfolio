...
# Ensure dev_tools is in path from container
sys.path.insert(0, '/workspace/cli')

try:
    from dev_tools.services.jules import JulesClient
except ImportError:
    print("Warning: dev_tools.services.jules not found in /workspace/cli", file=sys.stderr)
    try:
        from cli.dev_tools.services.jules import JulesClient
    except ImportError:
        print("Warning: failed to import JulesClient", file=sys.stderr)
        sys.exit(0)
