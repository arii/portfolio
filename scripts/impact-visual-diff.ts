...
import { logHeartbeat } from './heartbeat';
...
import { DEFAULT_VIEWPORTS, MAX_ROUTES_TO_REVIEW } from './visualReviewConstants';
...
async function main(): Promise<void> {
  ...
  const contentUrls = impact.changedFiles
    .filter(f => f.startsWith('src/content/research/') && f.endsWith('.md'))
    .map(f => `/devai/${path.basename(f, '.md')}`);
