...
PAGES = [
    "Home", "About", "Research", "DevAI", "Resume"
]

CONTENT_DIR = "src/content/research"
ARTICLES = []
if os.path.exists(CONTENT_DIR):
    for f in os.listdir(CONTENT_DIR):
        if f.endswith(".md"):
            ARTICLES.append(f.replace(".md", ""))

ALL_TARGETS = [("page", p) for p in PAGES] + [("article", a) for a in ARTICLES]

ISSUE_TEMPLATE = """# Visual Defect & Design Improvement: {title}

## Visual Defect Description
The current design aesthetics for `{filename}` rely on formulaic AI-generator boilerplate patterns that dilute the high-end engineering portfolio aesthetic.
Specifically:
...
## Validation Steps
1. Apply the new CSS rules and component structural changes in `{filepath}`.
...
def generate_issues():
    for target_type, target_name in ALL_TARGETS:
        filename = f"{target_name}.tsx" if target_type == "page" else f"{target_name}.md"
        filepath = f"src/pages/{filename}" if target_type == "page" else f"src/content/research/{filename}"
        issue_title = f"{target_name.capitalize()}"
        issue_filename = f"design-issue-{target_name.lower()}.md"
