    ...
    <main className="space-y-10 max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-1">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-slate-100 tracking-tight">{HERO_DATA.name}</h1>
        ...
        {/* Career & Education Badge Strip */}
        <div className="flex flex-wrap items-center gap-2 pt-2 text-xs sm:text-sm font-semibold text-slate-300">
          {HERO_DATA.careerBadges.map((badge, idx) => (
            <React.Fragment key={badge}>
              <span className="px-2.5 py-1 rounded-md bg-slate-800/80 border border-slate-700/60 text-slate-200">
                {badge}
              </span>
              {idx < HERO_DATA.careerBadges.length - 1 && <span className="text-slate-600">·</span>}
            </React.Fragment>
          ))}
        </div>
      ...
      <section className="space-y-4 pt-4 border-t border-slate-800/80">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {PHILOSOPHY_TENETS.map((tenet) => (
            <div
              key={tenet.id}
              className="bg-slate-900/30 border border-slate-800/80 rounded-lg p-5 space-y-2 hover:border-slate-700/80 transition-colors"
            >
      ...
      {/* Action Tile Navigation Cards */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {FOCUS_AREAS.map((item) => (
          <div
            key={item.id}
            onClick={() => handleNav(item.id)}
            className="group bg-slate-900/30 hover:bg-slate-900/60 border border-slate-800/80 hover:border-amber-500/40 rounded-lg p-5 cursor-pointer transition-all duration-200 flex flex-col justify-between space-y-4"
          >
