...
const MermaidDiagram: React.FC<{ chart: string }> = ({ chart }) => {
  ...
  useEffect(() => {
    ...
    const renderChart = async () => {
      try {
        ...
        if (isMounted) {
          setSvgContent(`<div class="text-accent text-sm p-4 border border-accent/20 rounded-lg bg-accent/5">Failed to render diagram</div>`);
        }
      }
    };
    renderChart();
  ...
  return (
    <div
      className="my-8 rounded-2xl border border-line bg-surface p-4 sm:p-6 overflow-x-auto flex justify-center mermaid-container"
