...
const HeroPathCard: React.FC<HeroPathCardProps> = ({ card, onNavigate }) => {
  ...
  return (
    <button
      onClick={handleClick}
      className="text-left w-full group bg-surface hover:bg-surface-alt border border-line hover:border-accent/50 rounded-xl p-5 sm:p-6 cursor-pointer transition-all duration-200 flex flex-col justify-between space-y-3.5 shadow-md focus:outline-none focus:ring-2 focus:ring-accent/50 h-full"
    >
