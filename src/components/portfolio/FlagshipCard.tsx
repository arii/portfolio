...
import { Activity, Server, FileText, ShoppingBag, Cpu, ExternalLink, ArrowRight, FlaskConical } from 'lucide-react';
...
export const FlagshipCard: React.FC<FlagshipCardProps> = ({
  tool,
  onNavigate,
  onImageClick,
}) => {
  ...
  const imageSrc =
    tool.id === 'hrm-flagship'
      ? 'assets/research/hrm-flagship.png'
      : tool.id === 'repo-auditor-ai'
      ? 'assets/research/repo-auditor-ai.png'
      : null;
        ...
        <div className="p-6 bg-[#020617] border-b border-line min-h-[140px] flex flex-col justify-center space-y-2">
          ...
          <div className="text-white font-black text-lg leading-tight font-display">
            {tool.customPreview.headline.map((line, idx) => (
              <span key={idx} className={line.accent ? 'text-accent' : ''}>
                {line.text}{' '}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-3">
          ...
          {tool.inDevMessage && (
            <div className="bg-[#0f172a] border border-line p-3 rounded-2xl text-xs flex gap-2 items-start text-text-dim">
              ...
              <p>
                <strong className="text-text-main">{tool.inDevMessage.highlight}</strong>{' '}
                {tool.inDevMessage.rest}
              </p>
            </div>
          )}
        </div>
            ...
            {tool.externalUrl ? (
              ...
              tool.canonicalPath && (
                <button
                  ...
                  className="inline-flex items-center space-x-1.5 bg-accent/10 border border-accent/20 px-3 py-1.5 rounded-xl text-xs font-semibold text-accent hover:bg-accent/20 transition-colors cursor-pointer"
                >
